...
#[test]
fn open_existing_writer_and_append_more_tokens() {
    let temp = tempdir().expect("create temp dir");
    let path = temp.path().join("cache-open-append.ncf-kvcache");
    let config = KvCacheConfig {
        layers: 1,
        heads: 1,
        element_bytes: 4,
    ...
    {
        let mut writer = KvCacheWriter::open(&path).expect("open existing writer");
        assert_eq!(writer.local_token_count(), 20);
        assert_eq!(writer.committed_token_count(), 20);
        for i in 20..40u32 {
            let frame = (0..config.frame_stride() as u8)
                .map(|b: u8| b.wrapping_add(i as u8))
                .collect::<Vec<u8>>();
    ...
    let reader = KvcacheReader::open(&path).expect("open reader");
    let token_bytes = reader.token_bytes(0, 0, 39).expect("last token available");
    assert_eq!(reader.visible_token_count(), 40);
    assert_eq!(token_bytes, &[39, 40, 41, 42]);
}
